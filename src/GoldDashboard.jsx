...
import { GOLD, GOLD_DARK, BG, SURFACE, SURFACE2, BORDER, TEXT, TEXT_DIM, BRANCHES, CATEGORIES, fmt, fmtAED, getThemeColors } from './constants';
import {
...
export default function GoldDashboard() {
  ...
  // ── Theme Colors ──
  const themeColors = getThemeColors(isLightTheme);
  ...
  const tooltipStyle = { background: themeColors.SURFACE2, border: `1px solid ${themeColors.BORDER}`, borderRadius: 8, color: themeColors.TEXT, fontSize: 11 };
  ...
  return (
    <div style={{
      ...
      {/* ── TOPBAR ── */}
      <div style={{
        background: themeColors.SURFACE, borderBottom: `1px solid ${themeColors.BORDER}`,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          ...
          <div>
            ...
            <div style={{ color: themeColors.TEXT_DIM, fontSize: 10, fontFamily: "'DM Mono',monospace", letterSpacing: "0.1em" }}>SALES INTELLIGENCE PLATFORM</div>
          </div>
        </div>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setActiveTab(t.id)} style={{
              ...
              border: activeTab === t.id ? "none" : `1px solid ${themeColors.BORDER}`,
              color: activeTab === t.id ? (isLightTheme ? "#000" : "#000") : themeColors.TEXT_DIM,
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
          ...
          <button
            ...
            onMouseEnter={e => {
              ...
              e.currentTarget.style.background = isLightTheme ? `${GOLD_DARK}22` : `${GOLD}22`;
            }}
            onMouseLeave={e => {
              e.currentTarget.style.borderColor = themeColors.BORDER;
              e.currentTarget.style.background = themeColors.SURFACE2;
            }}
            title={isLightTheme ? "Switch to Dark Theme" : "Switch to Light Theme"}
          >
            <span style={{ fontSize: 16 }}>{isLightTheme ? "🌙" : "☀️"}</span>
            <span style={{ color: themeColors.TEXT, fontSize: 11, fontFamily: "'DM Mono',monospace" }}>
              {isLightTheme ? "Dark" : "Light"}
            </span>
          </button>
          
          <div style={{ width: 1, height: 30, background: themeColors.BORDER }} />
          <div style={{ textAlign: "right" }}>
            <div style={{ color: themeColors.TEXT, fontSize: 13, fontFamily: "'DM Mono',monospace" }}>
              {clock.toLocaleTimeString("en-AE", { hour: "2-digit", minute: "2-digit", second: "2-digit" })}
            </div>
            <div style={{ color: themeColors.TEXT_DIM, fontSize: 10, fontFamily: "'DM Mono',monospace" }}>Dubai / UTC+4</div>
          </div>
          <div style={{ width: 1, height: 30, background: themeColors.BORDER }} />
          <div style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
            ...
            <div>
              <div style={{ color: themeColors.TEXT, fontSize: 12, fontWeight: 500 }}>Regional Manager</div>
              <div style={{ color: themeColors.TEXT_DIM, fontSize: 10, fontFamily: "'DM Mono',monospace" }}>{branchCountLabel}</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <div className="scroll-container" style={{ flex: 1, padding: 24 }}>
          ...
          <div style={{
            ...
            <div style={{ color: themeColors.TEXT_DIM, fontSize: 11, fontFamily: "'DM Mono',monospace", marginRight: 4 }}>FILTER:</div>
            {["All", "24K", "18K"].map(c => (
              <button key={c} onClick={() => setFilterCarat(c)} style={{
                background: filterCarat === c ? `${GOLD}22` : "transparent",
                border: `1px solid ${filterCarat === c ? GOLD : themeColors.BORDER}`,
                color: filterCarat === c ? GOLD : themeColors.TEXT_DIM,
                borderRadius: 6, padding: "5px 12px", cursor: "pointer", fontSize: 11,
                fontFamily: "'DM Mono',monospace", transition: "all 0.15s",
              }}>{c}</button>
            ))}
            <div style={{ width: 1, height: 20, background: themeColors.BORDER }} />
            {["All", ...CATEGORIES].map(c => (
              <button key={c} onClick={() => setFilterCategory(c)} style={{
                background: filterCategory === c ? `${GOLD}22` : "transparent",
                border: `1px solid ${filterCategory === c ? GOLD : themeColors.BORDER}`,
                color: filterCategory === c ? GOLD : themeColors.TEXT_DIM,
                borderRadius: 6, padding: "5px 12px", cursor: "pointer", fontSize: 11,
                transition: "all 0.15s",
              }}>{c}</button>
            ))}
            <div style={{ marginLeft: "auto", display: "flex", gap: 8 }}>
              <select
                ...
                style={{ background: themeColors.SURFACE, border: `1px solid ${themeColors.BORDER}`, color: themeColors.TEXT, borderRadius: 6, padding: "5px 10px", fontSize: 11, fontFamily: "'DM Mono',monospace" }}
              >
              ...
              <select
                ...
                style={{ background: themeColors.SURFACE, border: `1px solid ${themeColors.BORDER}`, color: themeColors.TEXT, borderRadius: 6, padding: "5px 10px", fontSize: 11 }}
              >
          ...
          {activeTab === "overview" && (
            <>
              ...
              <div style={{ display: "flex", gap: 12, marginBottom: 22, flexWrap: "wrap" }}>
                <KPICard label="Total Sales" value={fmtAED(metrics.totalSales)} sub={`${branchCountLabel}${dateLabel}`} delta={8.4} icon="💰" highlight pulse={dataPulse} themeColors={themeColors} />
                <KPICard label="Gold Sold" value={`${(metrics.totalGrams / 1000).toFixed(2)} kg`} sub={`${metrics.totalGrams.toLocaleString()}g total`} delta={5.1} icon="⚖️" pulse={dataPulse} themeColors={themeColors} />
                {(filterCarat === "All" || filterCarat === "24K") && (
                  <KPICard label="24K Revenue" value={fmtAED(metrics.total24K)} sub={`${metrics.caratPct24K}% of total`} delta={11.2} icon="🥇" pulse={dataPulse} themeColors={themeColors} />
                )}
                {(filterCarat === "All" || filterCarat === "18K") && (
                  <KPICard label="18K Revenue" value={fmtAED(metrics.total18K)} sub={`${metrics.caratPct18K}% of total`} delta={3.8} icon="🥈" pulse={dataPulse} themeColors={themeColors} />
                )}
                <KPICard label="Transactions" value={metrics.totalTxns.toLocaleString()} sub={`Avg ${fmtAED(metrics.totalTxns > 0 ? Math.round(metrics.totalSales / metrics.totalTxns) : 0)}`} delta={6.3} icon="🧾" pulse={dataPulse} themeColors={themeColors} />
                <KPICard label="Low Stock Alerts" value={metrics.lowStockCount} sub="Branches need reorder" icon="⚠️" delta={-2} pulse={dataPulse} themeColors={themeColors} />
              </div>

              {/* Charts Row 1 */}
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 280px", gap: 16, marginBottom: 16 }}>
                {/* Hourly Trend */}
                <div style={{ background: themeColors.SURFACE, border: `1px solid ${themeColors.BORDER}`, borderRadius: 12, padding: 20 }}>
                  <SectionHeader action="View All" onAction={() => setViewAllModal('hourly')} themeColors={themeColors}>Hourly Sales Trend{filterCarat !== "All" ? ` (${filterCarat})` : ""}</SectionHeader>
                  <ResponsiveContainer width="100%" height={180}>
                    <AreaChart data={filteredHourlyData}>
                      ...
                      <XAxis dataKey="hour" stroke={themeColors.TEXT_DIM} tick={{ fontSize: 9, fontFamily: "'DM Mono',monospace" }} interval={3} />
                      <YAxis stroke={themeColors.TEXT_DIM} tick={{ fontSize: 9 }} tickFormatter={v => fmt(v)} />
                ...
                {/* Carat Comparison */}
                <div style={{ background: themeColors.SURFACE, border: `1px solid ${themeColors.BORDER}`, borderRadius: 12, padding: 20 }}>
                  <SectionHeader themeColors={themeColors}>
                    <div style={{ display: "flex", gap: 8 }}>
                      {["daily", "weekly", "monthly"].map(v => (
                        <button key={v} onClick={() => setTimeView(v)} style={{
                          background: timeView === v ? `${GOLD}22` : "transparent",
                          border: `1px solid ${timeView === v ? GOLD : themeColors.BORDER}`,
                          color: timeView === v ? GOLD : themeColors.TEXT_DIM, borderRadius: 5,
                  ...
                  <ResponsiveContainer width="100%" height={180}>
                    <BarChart data={timeView === "monthly" ? filteredMonthlyData : filteredDailyData}>
                      <CartesianGrid strokeDasharray="3 3" stroke={themeColors.BORDER} />
                      <XAxis dataKey={timeView === "monthly" ? "month" : "day"} stroke={themeColors.TEXT_DIM} tick={{ fontSize: 9 }} />
                      <YAxis stroke={themeColors.TEXT_DIM} tick={{ fontSize: 9 }} tickFormatter={v => fmt(v)} />
                ...
                {/* Carat Pie — DYNAMIC */}
                <div style={{ background: themeColors.SURFACE, border: `1px solid ${themeColors.BORDER}`, borderRadius: 12, padding: 20 }}>
                  <SectionHeader themeColors={themeColors}>Carat Split</SectionHeader>
                    ...
                    {caratShare.map(c => (
                      <div key={c.name} style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          ...
                          <span style={{ color: themeColors.TEXT_DIM, fontSize: 11 }}>{c.name}</span>
                        </div>
                        <span style={{ color: themeColors.TEXT, fontSize: 12, fontFamily: "'DM Mono',monospace", fontWeight: 600 }}>{c.value}%</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {/* Top Branches */}
              <div style={{ background: themeColors.SURFACE, border: `1px solid ${themeColors.BORDER}`, borderRadius: 12, padding: 20, marginBottom: 16 }}>
                <SectionHeader action={`View All ${filteredBranches.length}`} onAction={() => setViewAllModal('branches')} themeColors={themeColors}>Top Performing Branches{filterCarat !== "All" ? ` (${filterCarat})` : ""}</SectionHeader>
                <div style={{ display: "flex", gap: 10, overflowX: "auto", paddingBottom: 4 }}>
                  {topBranches.slice(0, 8).map((b, i) => {
                    ...
                    return (
                      <div key={b.id} style={{
                        background: themeColors.SURFACE2, border: `1px solid ${themeColors.BORDER}`,
                        ...
                        <div style={{ color: themeColors.TEXT_DIM, fontSize: 9, fontFamily: "'DM Mono',monospace", marginBottom: 4 }}>#{i + 1}</div>
                        <div style={{ color: themeColors.TEXT, fontSize: 12, fontWeight: 600, marginBottom: 10, lineHeight: 1.3 }}>{b.name}</div>
                        <div style={{ background: themeColors.BORDER, borderRadius: 4, height: 4, marginBottom: 8, overflow: "hidden" }}>
                        ...
                        <div style={{ color: themeColors.TEXT_DIM, fontSize: 10, marginTop: 2 }}>{b.transactions} txns</div>
                      </div>
                    );
                  })}
                </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                {/* Top Selling */}
                <div style={{ background: themeColors.SURFACE, border: `1px solid ${themeColors.BORDER}`, borderRadius: 12, padding: 20 }}>
                  <SectionHeader themeColors={themeColors}>Top Selling Items{filterCategory !== "All" ? ` — ${filterCategory}` : ""}{filterCarat !== "All" ? ` (${filterCarat})` : ""}</SectionHeader>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                    <thead>
                      <tr style={{ borderBottom: `1px solid ${themeColors.BORDER}` }}>
                        {["Item", "Carat", "Qty", "Revenue", "Days to Sell"].map(h => (
                          <th key={h} style={{ padding: "8px 10px", color: themeColors.TEXT_DIM, fontFamily: "'DM Mono',monospace", fontSize: 9, textAlign: "left", textTransform: "uppercase", letterSpacing: "0.05em" }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {topItems.map((item, i) => (
                        <tr key={i} style={{ borderBottom: `1px solid ${themeColors.BORDER}` }}>
                          <td style={{ padding: "9px 10px", color: themeColors.TEXT }}>{item.category}</td>
                          <td style={{ padding: "9px 10px", color: item.carat === "24K" ? GOLD : GOLD_DARK, fontFamily: "'DM Mono',monospace" }}>{item.carat}</td>
                          <td style={{ padding: "9px 10px", color: themeColors.TEXT }}>{item.qty}</td>
                      ...
                      {topItems.length === 0 && (
                        <tr><td colSpan={5} style={{ padding: 20, color: themeColors.TEXT_DIM, textAlign: "center" }}>No items match the current filters</td></tr>
                      )}
                    </tbody>
                  </table>
                </div>
                ...
                {/* Slow Moving */}
                <div style={{ background: themeColors.SURFACE, border: `1px solid ${themeColors.BORDER}`, borderRadius: 12, padding: 20 }}>
                  <SectionHeader themeColors={themeColors}>Slow Moving Inventory{filterCategory !== "All" ? ` — ${filterCategory}` : ""}</SectionHeader>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                    <thead>
                      <tr style={{ borderBottom: `1px solid ${themeColors.BORDER}` }}>
                    ...
                    <tbody>
                      {slowItems.map((item, i) => (
                        <tr key={i} style={{ borderBottom: `1px solid ${themeColors.BORDER}` }}>
                          ...
                          <td style={{ padding: "9px 10px", color: themeColors.TEXT }}>{item.stock} pcs</td>
                          <td style={{ padding: "9px 10px", color: themeColors.TEXT_DIM, fontSize: 10 }}>{item.branch.split(" ").slice(0, 2).join(" ")}</td>
                        </tr>
                      ))}
                      {slowItems.length === 0 && (
                        <tr><td colSpan={5} style={{ padding: 20, color: themeColors.TEXT_DIM, textAlign: "center" }}>No items match the current filters</td></tr>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginTop: 16 }}>
                <div style={{ background: themeColors.SURFACE, border: `1px solid ${GOLD_DARK}`, borderRadius: 12, padding: 20 }}>
                  <SectionHeader action={`${aiData.reorderSuggestions.length} items`} themeColors={themeColors}>🤖 AI Reorder Suggestions</SectionHeader>
                  {aiData.reorderSuggestions.length === 0 ? (
                    ...
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                      <thead><tr style={{ borderBottom: `1px solid ${themeColors.BORDER}` }}>
                        {["Branch", "Carat", "Current", "Order Qty", "Urgency"].map(h => (
                          <th key={h} style={{ padding: "8px 10px", color: themeColors.TEXT_DIM, fontFamily: "'DM Mono',monospace", fontSize: 9, textAlign: "left", textTransform: "uppercase" }}>{h}</th>
                        ))}
                      </tr></thead>
                      <tbody>
                        {aiData.reorderSuggestions.map((r, i) => (
                          <tr key={i} style={{ borderBottom: `1px solid ${themeColors.BORDER}` }}>
                            <td style={{ padding: "9px 10px", color: themeColors.TEXT }}>{r.branch}</td>
                ...
                <div style={{ background: themeColors.SURFACE, border: `1px solid ${GOLD_DARK}`, borderRadius: 12, padding: 20 }}>
                  <SectionHeader action="AI-Optimized" themeColors={themeColors}>🔄 Branch Transfer Opportunities</SectionHeader>
                      ...
                      <tbody>
                        {aiData.transferOps.map((t, i) => (
                          <tr key={i} style={{ borderBottom: `1px solid ${themeColors.BORDER}` }}>
                            ...
                            <td style={{ padding: "9px 10px", color: themeColors.TEXT, fontFamily: "'DM Mono',monospace" }}>{t.qty}g</td>
          ...
          {activeTab === "branches" && (
            <>
              {selectedBranch !== null && branchData[selectedBranch] && (
                <BranchDetail branch={branchData[selectedBranch]} onClose={() => setSelectedBranch(null)} themeColors={themeColors} />
              )}
              <div style={{ background: themeColors.SURFACE, border: `1px solid ${themeColors.BORDER}`, borderRadius: 12, padding: 20 }}>
                <SectionHeader themeColors={themeColors}>All {filteredBranches.length} Dubai Branches — Click to Drill Down</SectionHeader>
                <BranchTable data={filteredBranches} onSelect={setSelectedBranch} selected={selectedBranch} themeColors={themeColors} />
              </div>
            </>
          )}
          ...
          {activeTab === "carats" && (
            <>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
                <div style={{ background: themeColors.SURFACE, border: `1px solid ${themeColors.BORDER}`, borderRadius: 12, padding: 20 }}>
                  <SectionHeader themeColors={themeColors}>24K vs 18K — Monthly Revenue</SectionHeader>
                  <ResponsiveContainer width="100%" height={250}>
                    <BarChart data={filteredMonthlyData}>
                      <CartesianGrid strokeDasharray="3 3" stroke={themeColors.BORDER} />
                      <XAxis dataKey="month" stroke={themeColors.TEXT_DIM} tick={{ fontSize: 10 }} />
                      <YAxis stroke={themeColors.TEXT_DIM} tick={{ fontSize: 10 }} tickFormatter={v => fmt(v)} />
                      <Tooltip contentStyle={tooltipStyle} formatter={v => [fmtAED(v)]} />
                ...
                <div style={{ background: themeColors.SURFACE, border: `1px solid ${themeColors.BORDER}`, borderRadius: 12, padding: 20 }}>
                  <SectionHeader themeColors={themeColors}>Carat Preference by Branch (Top 10)</SectionHeader>
                  <ResponsiveContainer width="100%" height={250}>
                    <BarChart data={topBranches} layout="vertical">
                      <CartesianGrid strokeDasharray="3 3" stroke={themeColors.BORDER} />
                      <XAxis type="number" stroke={themeColors.TEXT_DIM} tick={{ fontSize: 9 }} tickFormatter={v => fmt(v)} />
                      <YAxis type="category" dataKey="shortName" stroke={themeColors.TEXT_DIM} tick={{ fontSize: 9 }} width={80} />
                      <Tooltip contentStyle={tooltipStyle} formatter={v => [fmtAED(v)]} />
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 12 }}>
                {[
                  ...
                  <div key={item.label} style={{ background: themeColors.SURFACE, border: `1px solid ${themeColors.BORDER}`, borderRadius: 12, padding: 20 }}>
                    <div style={{ color: themeColors.TEXT_DIM, fontSize: 10, fontFamily: "'DM Mono',monospace", marginBottom: 8, letterSpacing: "0.06em" }}>{item.label.toUpperCase()}</div>
                    <div style={{ color: item.c, fontSize: 28, fontWeight: 700, fontFamily: "'Playfair Display',serif" }}>{item.val}</div>
                    <div style={{ color: themeColors.TEXT_DIM, fontSize: 11, marginTop: 4 }}>{item.sub}</div>
                  </div>
                ))}
              </div>
            </>
          ...
          {activeTab === "time" && (
            <>
              <div style={{ background: themeColors.SURFACE, border: `1px solid ${themeColors.BORDER}`, borderRadius: 12, padding: 20, marginBottom: 16 }}>
                <SectionHeader themeColors={themeColors}>Peak Hour Analysis{filterCarat !== "All" ? ` (${filterCarat})` : ""}</SectionHeader>
                  <ResponsiveContainer width="100%" height={220}>
                    <AreaChart data={filteredHourlyData}>
                      ...
                      <CartesianGrid strokeDasharray="3 3" stroke={themeColors.BORDER} />
                      <XAxis dataKey="hour" stroke={themeColors.TEXT_DIM} tick={{ fontSize: 10, fontFamily: "'DM Mono',monospace" }} />
                      <YAxis stroke={themeColors.TEXT_DIM} tick={{ fontSize: 10 }} tickFormatter={v => fmt(v)} />
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                <div style={{ background: themeColors.SURFACE, border: `1px solid ${themeColors.BORDER}`, borderRadius: 12, padding: 20 }}>
                  <SectionHeader themeColors={themeColors}>Daily Sales Pattern</SectionHeader>
                  <ResponsiveContainer width="100%" height={200}>
                    <BarChart data={filteredDailyData}>
                      <CartesianGrid strokeDasharray="3 3" stroke={themeColors.BORDER} />
                      <XAxis dataKey="day" stroke={themeColors.TEXT_DIM} tick={{ fontSize: 10 }} />
                      <YAxis stroke={themeColors.TEXT_DIM} tick={{ fontSize: 10 }} tickFormatter={v => fmt(v)} />
                      <Tooltip contentStyle={tooltipStyle} formatter={v => [fmtAED(v)]} />
                      {(filterCarat === "All" || filterCarat === "24K") && <Bar dataKey="24K" fill={GOLD} radius={[3, 3, 0, 0]} />}
                      {(filterCarat === "All" || filterCarat === "18K") && <Bar dataKey="18K" fill={GOLD_DARK} radius={[3, 3, 0, 0]} />}
                ...
                <div style={{ background: themeColors.SURFACE, border: `1px solid ${themeColors.BORDER}`, borderRadius: 12, padding: 20 }}>
                  <SectionHeader themeColors={themeColors}>Monthly Trend vs Target</SectionHeader>
                  <ResponsiveContainer width="100%" height={200}>
            ...
            <>
              <div style={{ display: "flex", gap: 12, marginBottom: 20 }}>
                {[
                  ...
                  <div key={s.label} style={{ flex: 1, background: themeColors.SURFACE, border: `1px solid ${themeColors.BORDER}`, borderRadius: 12, padding: "18px 20px" }}>
                    <div style={{ color: themeColors.TEXT_DIM, fontSize: 10, fontFamily: "'DM Mono',monospace", marginBottom: 8 }}>{s.label.toUpperCase()}</div>
                    <div style={{ color: s.c, fontSize: 32, fontWeight: 700, fontFamily: "'Playfair Display',serif" }}>{s.count}</div>
                    <div style={{ color: themeColors.TEXT_DIM, fontSize: 11, marginTop: 2 }}>branches</div>
                  </div>
                ))}
              </div>

              <div style={{ background: themeColors.SURFACE, border: `1px solid ${themeColors.BORDER}`, borderRadius: 12, padding: 20 }}>
                <SectionHeader themeColors={themeColors}>Branch-wise Stock Levels</SectionHeader>
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                    <thead>
                      <tr style={{ borderBottom: `1px solid ${themeColors.BORDER}` }}>
                        {["Branch", "24K Stock", "24K Reorder", "18K Stock", "18K Reorder", "Status", "Inventory Age"].map(h => (
                          <th key={h} style={{ padding: "10px 12px", color: themeColors.TEXT_DIM, fontFamily: "'DM Mono',monospace", fontSize: 9, textAlign: "left", textTransform: "uppercase", letterSpacing: "0.05em" }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {filteredBranches.map(b => {
                        ...
                        return (
                          <tr key={b.id} style={{ borderBottom: `1px solid ${themeColors.BORDER}` }}>
                            <td style={{ padding: "10px 12px", color: themeColors.TEXT }}>{b.name}</td>
                            <td style={{ padding: "10px 12px", color: isLow24K ? "#FF4444" : GOLD, fontFamily: "'DM Mono',monospace" }}>{b.stock24K}g</td>
                            <td style={{ padding: "10px 12px", color: themeColors.TEXT_DIM }}>{b.reorderLevel24K}g</td>
                            <td style={{ padding: "10px 12px", color: isLow18K ? "#FF4444" : GOLD_DARK, fontFamily: "'DM Mono',monospace" }}>{b.stock18K}g</td>
                            <td style={{ padding: "10px 12px", color: themeColors.TEXT_DIM }}>{b.reorderLevel18K}g</td>
                            ...
                            <td style={{ padding: "10px 12px", color: b.inventoryAge > 30 ? "#FF8C69" : themeColors.TEXT_DIM, fontFamily: "'DM Mono',monospace" }}>{b.inventoryAge}d</td>
                          </tr>
                        );
                      })}
                    </tbody>
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
            {[
              ...
              { label: 'Total Grams', val: `${(metrics.totalGrams / 1000).toFixed(2)} kg`, c: themeColors.TEXT, icon: '⚖️' },
              { label: 'Total Transactions', val: metrics.totalTxns.toLocaleString(), c: themeColors.TEXT, icon: '🧾' },
              { label: 'Branches Tracked', val: topBranches.length, c: GOLD, icon: '🏬' },
            ].map(s => (
              <div key={s.label} style={{ flex: 1, background: themeColors.SURFACE, border: `1px solid ${themeColors.BORDER}`, borderRadius: 10, padding: '14px 16px' }}>
                <div style={{ fontSize: 18, marginBottom: 4 }}>{s.icon}</div>
                <div style={{ color: themeColors.TEXT_DIM, fontSize: 9, fontFamily: "'DM Mono',monospace", marginBottom: 4, letterSpacing: '0.06em' }}>{s.label.toUpperCase()}</div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
            {topBranches.map((b, i) => {
              ...
              return (
                <div key={b.id} style={{
                  background: i < 3 ? (isLightTheme ? themeColors.SURFACE2 : '#1A1505') : themeColors.SURFACE, border: `1px solid ${i < 3 ? GOLD_DARK : themeColors.BORDER}`,
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                    <div style={{
                      color: i < 3 ? GOLD : themeColors.TEXT_DIM, fontSize: 10, fontFamily: "'DM Mono',monospace",
                      background: i < 3 ? `${GOLD}15` : `${themeColors.BORDER}88`, padding: '2px 8px', borderRadius: 4,
                    }}>#{i + 1}</div>
                  ...
                  <div style={{ color: themeColors.TEXT, fontSize: 13, fontWeight: 600, marginBottom: 10, lineHeight: 1.3 }}>{b.name}</div>
                  <div style={{ background: themeColors.BORDER, borderRadius: 4, height: 4, marginBottom: 10, overflow: 'hidden' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 4 }}>
                    <div><div style={{ color: themeColors.TEXT_DIM, fontSize: 8, fontFamily: "'DM Mono',monospace" }}>TXNS</div><div style={{ color: themeColors.TEXT, fontSize: 12 }}>{b.transactions}</div></div>
                    <div><div style={{ color: themeColors.TEXT_DIM, fontSize: 8, fontFamily: "'DM Mono',monospace" }}>GRAMS</div><div style={{ color: themeColors.TEXT, fontSize: 12 }}>{(b.grams24K + b.grams18K).toLocaleString()}g</div></div>
                    <div><div style={{ color: themeColors.TEXT_DIM, fontSize: 8, fontFamily: "'DM Mono',monospace" }}>AVG ORDER</div><div style={{ color: themeColors.TEXT, fontSize: 12 }}>{fmtAED(b.avgOrderValue)}</div></div>
                    <div><div style={{ color: themeColors.TEXT_DIM, fontSize: 8, fontFamily: "'DM Mono',monospace" }}>FAST ITEM</div><div style={{ color: '#4ECDC4', fontSize: 12 }}>{b.fastMoving}</div></div>
                  </div>
                </div>
              );
            })}
          ...
          <div style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
            {(() => {
              ...
              return [
                ...
                { label: 'Hourly Average', val: fmtAED(avg), c: themeColors.TEXT, icon: '📈' },
              ].map(s => (
                <div key={s.label} style={{ flex: 1, background: themeColors.SURFACE, border: `1px solid ${themeColors.BORDER}`, borderRadius: 10, padding: '14px 16px' }}>
                  <div style={{ fontSize: 18, marginBottom: 4 }}>{s.icon}</div>
                  <div style={{ color: themeColors.TEXT_DIM, fontSize: 9, fontFamily: "'DM Mono',monospace", marginBottom: 4, letterSpacing: '0.06em' }}>{s.label.toUpperCase()}</div>
                  <div style={{ color: s.c, fontSize: 22, fontWeight: 700, fontFamily: "'Playfair Display',serif" }}>{s.val}</div>
                  {s.sub && <div style={{ color: themeColors.TEXT_DIM, fontSize: 11, marginTop: 2 }}>{s.sub}</div>}
                </div>
              ));
            })()}
          </div>
          {/* Full-width hourly area chart */}
          <div style={{ background: themeColors.SURFACE, border: `1px solid ${themeColors.BORDER}`, borderRadius: 12, padding: 24, marginBottom: 24 }}>
            <ResponsiveContainer width="100%" height={320}>
              <AreaChart data={filteredHourlyData}>
                ...
                <CartesianGrid strokeDasharray="3 3" stroke={themeColors.BORDER} />
                <XAxis dataKey="hour" stroke={themeColors.TEXT_DIM} tick={{ fontSize: 11, fontFamily: "'DM Mono',monospace" }} />
                <YAxis stroke={themeColors.TEXT_DIM} tick={{ fontSize: 11 }} tickFormatter={v => fmt(v)} />
                <Tooltip contentStyle={tooltipStyle} formatter={v => [fmtAED(v), 'Revenue']} />
                <Area type="monotone" dataKey="sales" stroke={filterCarat === '18K' ? GOLD_DARK : GOLD} fill="url(#goldGradModal)" strokeWidth={2.5} dot={{ r: 3, fill: GOLD, stroke: themeColors.BG, strokeWidth: 2 }} activeDot={{ r: 6, fill: GOLD }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          {/* Hourly breakdown table */}
          <div style={{ background: themeColors.SURFACE, border: `1px solid ${themeColors.BORDER}`, borderRadius: 12, padding: 20 }}>
            <div style={{ color: themeColors.TEXT_DIM, fontSize: 10, fontFamily: "'DM Mono',monospace", marginBottom: 12, letterSpacing: '0.06em' }}>HOUR-BY-HOUR BREAKDOWN</div>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 8 }}>
              {filteredHourlyData.map((h, i) => {
                ...
                return (
                  <div key={i} style={{
                    background: isPeak ? (isLightTheme ? themeColors.SURFACE2 : '#1A1505') : themeColors.SURFACE2, border: `1px solid ${isPeak ? GOLD_DARK : themeColors.BORDER}`,
                    borderRadius: 8, padding: '10px 12px', textAlign: 'center',
                    opacity: isActive ? 1 : 0.4,
                  }}>
                    <div style={{ color: isPeak ? GOLD : themeColors.TEXT_DIM, fontSize: 10, fontFamily: "'DM Mono',monospace", marginBottom: 4 }}>{h.hour}</div>
                    <div style={{ color: isPeak ? GOLD : themeColors.TEXT, fontSize: 13, fontWeight: 600, fontFamily: "'Playfair Display',serif" }}>{fmtAED(h.sales)}</div>
      ...
      {/* FOOTER */}
      <div style={{
        background: themeColors.SURFACE, borderTop: `1px solid ${themeColors.BORDER}`,
        padding: "10px 24px", display: "flex", justifyContent: "space-between", alignItems: "center",
        fontSize: 10, color: themeColors.TEXT_DIM, fontFamily: "'DM Mono',monospace", flexShrink: 0,
      }}>
